...
{
    bool hor = ((row < toRow && row > fromRow) ||
                (row > toRow && row < fromRow));

    bool ver = ((col < toCol && col > fromCol) ||
                (col > toCol && col < fromCol));
...
void Piece::notify(Subject<Info, State> &whoFrom)
{
    ...
    if (whoFrom.getState().type == StateType::Check && whoFrom.getState().colour != colour && colour != Colour::NoColour && (whoFrom.getInfo().rank == Rank::blank || whoFrom.getInfo().rank == Rank::k || rank == Rank::n )
        && (inBetween(whoFrom.getInfo().row, whoFrom.getInfo().col, row, col, whoFrom.getState().row, whoFrom.getState().col) || whoFrom.getInfo().rank == Rank::k ||  rank == Rank::n) && (row != whoFrom.getState().row || col != whoFrom.getState().col))
    {
